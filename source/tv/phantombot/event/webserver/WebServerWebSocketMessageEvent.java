...
/**
 * A message received from the bots web server via web socket
 *
 * @author gmt2001
 */
public final class WebServerWebSocketMessageEvent extends WebServerMessageEvent {
    /**
     ...
     */
    public WebServerWebSocketMessageEvent(ChannelHandlerContext channelHandlerContext, String source) {
     ...
     */
    public WebServerWebSocketMessageEvent(ChannelHandlerContext channelHandlerContext, JSONObject jo) {
    ...
    protected void doSendResponse(HttpResponseStatus status, String json) {
        if (!this.channelHandlerContext.refersTo(null) && this.channelHandlerContext.get().channel().isActive()) {
            WebSocketFrameHandler.sendWsFrame(this.channelHandlerContext.get(), null,
